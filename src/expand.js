...
export function buildRawFromSimplified(simplified) {
	...
	// Preserve name records the info model does not own — localized (non-English)
	// copies of known IDs, unknown IDs (15/18/20/23/24/26), and custom 256+
	// records in static fonts — so a binary → JSON → binary round-trip keeps
	// them. Variable fonts regenerate the 256+ range from fvar/STAT below, so
	// those are intentionally not carried forward.
	if (simplified.tables?.name?.names) {
		const isVariable = !!(simplified.axes && simplified.axes.length > 0);
		tables.name = mergePreservedNameRecords(
			tables.name,
			simplified.tables.name,
			isVariable,
		);
	}
...
/**
 * Resolve the bold/italic style flags used to coordinate head.macStyle and
 * OS/2.fsSelection.
 *
 * Follows the RIBBI rule: the bold bit is set only when the weight is
 * *exactly* 700, so ExtraBold/Black (800/900) faces do not receive
 * contradictory bits across the two tables. Explicit `info.isBold` /
 * `info.isItalic` flags override the derived values.
 */
function resolveStyleFlags(font) {
	const isBold =
		font.isBold !== undefined
			? !!font.isBold
			: (font.weightClass || 400) === 700;
	const isItalic =
		font.isItalic !== undefined
			? !!font.isItalic
...
function buildHeadTable(font, metrics) {
	let macStyle;
	if (font.macStyle !== undefined) {
		// Honor an explicit macStyle (e.g. preserved on import or set by the author).
		macStyle = font.macStyle;
	} else {
		const { isBold, isItalic } = resolveStyleFlags(font);
		macStyle = 0;
...
function buildNameTable(font) {
	...
	// Typographic family/subfamily (name IDs 16/17). Per OpenType guidance,
	// emit them only when they differ from the legacy family/subfamily
	// (IDs 1/2); this is what lets multi-weight families group correctly in
	// modern apps. Never synthesize them — write only what the author or the
	// imported font supplied.
	if (
		font.typographicFamily &&
		font.typographicFamily !== (font.familyName || '')
	) {
		entries[16] = font.typographicFamily;
	}
	if (
		font.typographicSubfamily &&
		font.typographicSubfamily !== (font.styleName || '')
	) {
		entries[17] = font.typographicSubfamily;
	}

	// WWS family/subfamily (name IDs 21/22) — used when the typographic family
	// carries non-WWS attributes (Display, Caption, Optical size, etc.).
	if (font.wwsFamily) entries[21] = font.wwsFamily;
	if (font.wwsSubfamily) entries[22] = font.wwsSubfamily;

	// Variations PostScript name prefix (name ID 25) for variable fonts.
	if (font.variationsPostScriptNamePrefix) {
		entries[25] = font.variationsPostScriptNamePrefix;
	}
...
/**
 * Merge name records preserved from an imported font into a freshly built name
 * table, without disturbing the info-driven records.
 *
 * Records are keyed by the full (platformID, encodingID, languageID, nameID)
 * tuple so that localized copies of a known ID (e.g. a Japanese family name at
 * nameID 1) survive even though the built table already carries the English
 * copy of that ID.
 *
 * @param {object} builtName    - name table produced by buildNameTable (info-driven)
 * @param {object} originalName - original parsed name table from the import
 * @param {boolean} isVariable  - true when the font has fvar axes (256+ range is
 *                                regenerated by fvar/STAT and must NOT be carried)
 * @returns {object} merged name table
 */
function mergePreservedNameRecords(builtName, originalName, isVariable) {
	if (!originalName?.names?.length) return builtName;

	const tupleKey = (r) =>
		`${r.platformID},${r.encodingID},${r.languageID},${r.nameID}`;

	const present = new Set(builtName.names.map(tupleKey));
	const names = builtName.names.slice();

	for (const rec of originalName.names) {
		// Variable fonts regenerate the user range (256+) via fvar/STAT.
		if (isVariable && rec.nameID >= 256) continue;
		// The info model already owns this exact record.
		if (present.has(tupleKey(rec))) continue;
		names.push({
		...
		present.add(tupleKey(rec));
	}

	const merged = { version: builtName.version, names };

	// Preserve version-1 language-tag records so carried records that reference
	// them (languageID >= 0x8000) remain valid.
	if (originalName.version === 1 && originalName.langTagRecords?.length) {
		merged.version = 1;
		merged.langTagRecords = originalName.langTagRecords.map((lt) => ({
			...lt,
		}));
	}
...
function buildPostScriptName(font) {
	// A PostScript name must be ASCII, contain no spaces or the reserved
	// characters '[](){}<>/%', and be at most 63 characters long.
	const sanitize = (s) =>
		(s || '')
			.replace(/[^\x21-\x7e]/g, '') // drop spaces and non-ASCII
			.replace(/[[\](){}<>/%]/g, ''); // drop reserved characters
	const family = sanitize(font.familyName);
	const style = sanitize(font.styleName) || 'Regular';
	return `${family}-${style}`.slice(0, 63);
}
